...
describe("Example Test Suite", () => {
  xit("should return true for 1 + 1 = 2", () => {
  ...
  it("should filter isRentable", (done) => {
    request(app).get("/api/example?isRentable=true").expect(200, done);
    // .end((err, res) => {
  ...
  it("should filter findBySize", (done) => {
    request(app).get("/api/example?findBySize=15x10").expect(200, done);
    // .end((err, res) => {
    //   console.log(res.body);
    //   done();
    // });
